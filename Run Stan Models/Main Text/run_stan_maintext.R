...
#  Zheng et al. group prevalence model (Reduced McCarty Data - Dichotomous) ---------------------------------------------

### Prepare Stan data ---------------------------------------------

mccartyDichotomousReduced_ZhengGP_StanData <-
  list(
    N = nrow(y_dichotomous),
    K = ncol(y_dichotomous),
    y = y_dichotomous
...
options(mc.cores = parallel::detectCores())
rstan_options(auto_write = TRUE)
mccartyDichotomousReduced_ZhengGP_StanMod <-
  mccartyDichotomous_ZhengGP_StanMod

### Run Stan model ---------------------------------------------

mccartyDichotomousReduced_ZhengGP_StanFit <-
  mccartyDichotomous_ZhengGP_StanMod$sample(
    data = mccartyDichotomousReduced_ZhengGP_StanData,
    seed = 123,
    chains = 4,
    refresh = 200,
    parallel_chains = 4,
...
mccartyDichotomousReduced_ZhengGP_StanFit$cmdstan_diagnose()

### Create stanfit object ---------------------------------------------

mccartyDichotomousReduced_ZhengGP_StanFitObj <-
  read_stan_csv(mccartyDichotomousReduced_ZhengGP_StanFit$output_files())

### Extract samples ---------------------------------------------

mccartyDichotomousReduced_ZhengGP_Extract <-
  rstan::extract(mccartyDichotomousReduced_ZhengGP_StanFitObj)
